...
-- import qualified Course.Applicative as A
...
instance Extend List where
  (<<=) ::
    ...
    -> List b
  (<<=) f as = f <$> subLists as
    where subLists Nil = Nil
          -- subLists ls = A.lift2 (:.) id (subLists . drop 1) $ ls
          subLists ls = ls :. subLists (drop 1 ls)
          -- subLists lall @ (_ :. ls) = lall :. (subLists $ ls)
